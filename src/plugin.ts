...
function validateToken(token: string, linkSecret: string): TokenValidationResult {
    try {
        ...
        const expectedSig = crypto.createHmac('sha256', linkSecret).update(payload).digest('base64url');
...
const plugin: ModulePlugin = {
    ...
    register(router: Router, logger: Logger) {
        ...
        router.get('/validate', (req: Request, res: Response) => {
            try {
                ...
                logger.warn(`/validate failed reason=${err instanceof Error ? err.message : String(err)}`);
                res.status(401).json({ error: 'Unauthorized' });
            }
        });
    },
